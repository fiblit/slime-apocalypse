...
int main() {
	...
	ai::replan(ai::std_NNai, game_loop_clock);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		animate_agents(ai::std_NNai, game_loop_clock->delta());
...
void ai::calc_LMP_force(Object * a, std::vector<Object *> NNai, std::vector<Object *> NNboids, std::vector<Object *> NNstatic, Cspace2D * a_cspace, GLfloat dt) {
    float speed = 1.0f; // x m/s
    glm::vec2 goal_vel;

    //if there is a plan
    if (a->ai.has_plan()) {
        a->ai.goal = lookahead(a, a->ai.goal, a_cspace);
    ...
    if (a->ai.method == ai_comp::Planner::BOID || a->ai.method == ai_comp::Planner::PACK)
        ...
        goal_F = 2.0f*(goal_vel - glm::vec2(a->dyn.vel));
    ...
    for (Object * b : NNai) {
        ...
        if (ttc > 5/*seconds*/)
            continue;
        ttc_F += ttc_forces(a, b, static_cast<float>(ttc));
    }
...
void animate_agents(std::vector<Object *> agents, GLfloat dt) {
    for (Object * a : agents)
        ai::calc_LMP_force(a, ai::std_NNai, ai::std_NNboids, ai::std_NNstatic, ai::std_cspace, dt);
...
//refactor
void ai::replan(std::vector<Object *> agents, Gtime::Timer * clock) {
    clock->pause();
